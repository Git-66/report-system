...
public class ReportServiceImpl implements ReportService {
    ...
    private void sendDirectRequests(ReportRequest request) {
        ...
        Future<ExcelResponse> excelFuture = executor.submit(() -> rs.postForEntity("http://localhost:8888/excel", request, ExcelResponse.class).getBody());
        Future<PDFResponse> pdfFuture = executor.submit(() -> rs.postForEntity("http://localhost:9999/pdf", request, PDFResponse.class).getBody());

        executor.submit(() -> {
            try {
                updateLocal(pdfFuture.get());
            } catch (Exception e) {
...
//            }
    ...
    public void deleteFileBodyByReqId(String reqId, FileType type) {
        ...
        if (type == FileType.PDF) {
            ...
            log.error("Mismatch file type " + type);
            return;
        }
    ...
    @Override
    public void deleteReportByReqIdFromRepo(String reqId) {
        reportRequestRepo.deleteById(reqId);
    }

    @Override
    public ReportVO getReportByReqId(String reqId) {
        return new ReportVO(reportRequestRepo.findById(reqId).orElseThrow());
    }
}
