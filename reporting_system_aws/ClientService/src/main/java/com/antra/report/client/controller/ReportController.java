...
public class ReportController {
    ...
    @GetMapping("/report/content/{reqId}")
    public ResponseEntity<GeneralResponse> getReport(@PathVariable String reqId) {
        log.info("Got Request to obtain report: {}", reqId);
        return ResponseEntity.ok(new GeneralResponse(reportService.getReportByReqId(reqId)));
    }
    ...
    public void deleteFile(@PathVariable String reqId) throws IOException {
        log.debug("Got Request to delete File - reqId: {}", reqId);
        reportService.deleteFileBodyByReqId(reqId, FileType.PDF);
        reportService.deleteFileBodyByReqId(reqId, FileType.EXCEL);
        reportService.deleteReportByReqIdFromRepo(reqId);
        log.debug("Deleted File: {}", reqId);
    }


    @PutMapping("/report/edit")
    public ResponseEntity<GeneralResponse> updateReportDirectly(@RequestBody @Validated ReportRequest request) throws IOException {
        log.info("Got Request to update report - sync: {}", request);
        deleteFile(request.getReqId());
        return ResponseEntity.ok(new GeneralResponse(reportService.generateReportsSync(request)));
    }
