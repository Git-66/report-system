...
function showEdit(reqId){
    $('#edit_report_model').modal('toggle');
    //
    // $.getJSON('/report/content/'+reqId,
    //     function (data, textStatus, jqXHR) {  // success callback
    //         console.info(data);
    //         console.info(data.data);
    //         console.info(JSON.stringify(data.data));
    //         document.getElementById('editInputData').value = JSON.stringify(data.data);
    //     },function(e){
    //         alert('error' + e.error);
    //     }
    // );
...
function validateEditInput(){
    try {
        return JSON.parse($('#editInputData').val());
    }catch(err) {
        alert("This is not a valid Json.");
        return "";
    }
